...
func main() {
	...
	sovDomains := []string{}
	if mode, domains, err := governor.LoadSovereigntyConfig(cfg.SovereigntyPath, sovMode); err == nil {
		sovMode = mode
		sovDomains = domains
		hclog.Infof("", "sovereignty: loaded from %s (rules=%d)", cfg.SovereigntyPath, len(governor.GetSovereigntyRules()))
	} else {
