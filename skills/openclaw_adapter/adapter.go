...
func (a *Adapter) doExecute(input skills.SkillInput) skills.SkillOutput {
	...
	if ocReq.SessionKey == "" {
		ocReq.SessionKey = input.TraceID
	}
	if input.Args != nil {
		ocReq.Args = input.Args
	}
...
// OpenClawRequest is the v3.13+ request format for POST /tools/invoke.
...
func hcToOpenClaw(hc map[string]any) *OpenClawRequest {
	args := hc["args"]
	if args == nil {
		args = map[string]any{}
	}
	sessionKey := ""
	if t, ok := hc["trace"].(string); ok {
		sessionKey = t
	}
...
func (a *Adapter) doRequest(traceID, endpoint string, body []byte, timeout time.Duration, start time.Time) skills.SkillOutput {
	...
	if token := strings.TrimSpace(os.Getenv("OPENCLAW_API_TOKEN")); token != "" {
